...
const useWindowedFrameSubs = (
	...
	console.log(subsWithSeconds);
	console.log(subsWithFrameNumbers);
...
export const PaginatedSubtitles: React.FC<{
	...
	useEffect(() => {
		...
		console.log(windowRef.current?.getBoundingClientRect().height);
		
		const zoom = (zoomMeasurer.current?.getBoundingClientRect().height as number) / ZOOM_MEASURER_SIZE;
		const linesRendered = (windowRef.current?.getBoundingClientRect().height as number) / (LINE_HEIGHT * zoom);
